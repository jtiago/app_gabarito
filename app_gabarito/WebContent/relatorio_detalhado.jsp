...
<link rel="stylesheet" type="text/css" href="css/relatorio.css?<%=((int)Math.random()*1000)%>">
...
<div id="tudo">
	...
	<div class="relatorio">
		...
		String resposta = daoCorreto.getFiltroSemestreTurma(
				request.getParameter("semestre"),request.getParameter("turma"));
		%>
		<center><a href="relatorio_detalhado_imprimir.jsp?semestre=<%=request.getParameter("semestre")%>&turma=<%=request.getParameter("turma")%>" target="_blank">Imprimir Rlatório</a></center>
		<center>Média Avaliada: <%=media%></center>
		<table>
			<tr>
					...
					for(Aluno aluno : listaAluno){
						...
						<tr>
							...
							<%
								...
								if(Double.parseDouble(qt_ac) >= media){
									conceito = "H";
								} else{
									conceito = "<b>NH</b>";
								}
